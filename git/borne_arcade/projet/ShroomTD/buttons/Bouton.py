...
class Bouton:
    def __init__(self, image, x:float, y:float, name:str=None, sprite=None, assigned_tower=None) -> None:
        ...
        self.name = name
        self.scaled = False
        self.scaled_image = pygame.transform.scale(self.image, (self.width*1.1, self.height*1.1))
    ...
    def draw_and_scale(self, window, offset=0) -> None:
        ...
        if self.scaled:
            window.blit(self.scaled_image, (self.x - self.width*0.06 + offset, self.y - self.height*0.06))
        else:
