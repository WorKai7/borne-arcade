...
class Escape_menu:
    def __init__(self, screen_size:tuple, music) -> None:
        ...
        self.image = pygame.transform.scale(pygame.image.load(os.path.join("assets", "other", "escape_menu_rect.png")).convert_alpha(), (self.screen_size[0]/2, self.screen_size[1]/1.2))
    ...
    def draw_menu(self, game) -> bool:
        ...
        Cette méthode gère l'affichage, les clics efféctués par le joueur ainsi que l'animation du menu

        - game: objet game correspondant à la partie courante"""

        filler = pygame.Surface(self.screen_size, pygame.SRCALPHA)
        pygame.draw.rect(filler, (0, 0, 0, 128), pygame.Rect(0, 0, self.screen_size[0], self.screen_size[1]))
        self.image_y = balayage_haut(game.window, self.screen_size[1]/9, self.image, [self.image_x, self.image_y], game)
            ...
            for event in pygame.event.get():
                if event.type == pygame.MOUSEBUTTONDOWN:
                    ...
                    if self.main_menu_button.rect.collidepoint(pygame.mouse.get_pos()):
                        play_sound("click.ogg", 1, self.sfx_state)
                        main_menu_confirm_menu = Main_menu_confirm_menu(self.screen_size, self.sfx_state)
        ...
        balayage_bas(game.window, self.screen_size[1], self.image, [self.image_x, self.image_y], game)
