...
class Menu:
    def __init__(self) -> None:
        ...
        pygame.display.set_icon(pygame.image.load(os.path.join("assets", "other", "icon.png")))
        
        try:
            ctypes.windll.shell32.SetCurrentProcessExplicitAppUserModelID(os.path.join("assets", "other", "icon.png"))
        ...
        fondu_ouvrir(self.window, (self.width/3.55, self.height/3.7), self.logo)
        ...
        self.map_choice_text = pygame.font.SysFont('arialrounded', self.height//11).render('Choisissez une carte', True, (0, 0, 0))
        
        with open(os.path.join("data", "medals.json"), "r") as medals_file:
            self.medals = json.load(medals_file)
        
        try:
            self.music = create_music(pygame.mixer.Sound(os.path.join("assets", "sounds", "musics", "groovy_tower.mp3")))
        except pygame.error:
            self.music = None
            ...
            if event.type == pygame.MOUSEBUTTONDOWN:
                ...
                if self.current_window == "main":
                    if self.quit_button.rect.collidepoint(pygame.mouse.get_pos()):
                        play_sound("click.ogg", 1, self.sfx_state)
                        quit_confirm_menu = Quit_confirm_menu((self.width, self.height), self.sfx_state)
                    ...
                    if self.encyclopedia_button.rect.collidepoint(pygame.mouse.get_pos()):
                        play_sound("click.ogg", 1, self.sfx_state)
                        fondu_fermer(self.window, (self.width, self.height), 0.5, 50)
                        encyclopedia = Encyclopedia((self.width, self.height), self.sfx_state, self.music_state, self.music)
                    ...
                    for choice_map in self.available_maps:
                        if choice_map.rect.collidepoint(pygame.mouse.get_pos()):
                            play_sound("click.ogg", 1, self.sfx_state)
                            difficulty_menu = Difficulty_menu((self.width, self.height), self.sfx_state)
                            ...
                            if difficulty is not None:
                                ...
                                game.run()
                            break
                            #NOOOOOON CEST TERRIBLE JAI MIS UN BREAK OSKOUR JAI PAS TROUVE DAUTRE SOLUTION OSKOUROSKOUR
                            #(en vrai ya une autre solution ce serais de faire plein de if au lieu d'un for mais vsy relou)
    ...
    def draw_menu(self) -> None:
        ...
        if self.current_window == "main":
            ...
            self.update_medals()
            self.window.blit(self.map_choice_text, (self.width/4.2, self.height/20))
            self.back_button.draw_and_scale(self.window)
            
            for choice_map in self.available_maps:
                choice_map.draw_and_scale(self.window)
                if self.medals[choice_map.name]["easy"]:
                    self.window.blit(self.bronze_medal, (choice_map.x + choice_map.width / 15, choice_map.y + choice_map.height*1.03))
                if self.medals[choice_map.name]["medium"]:
                    self.window.blit(self.silver_medal, (choice_map.x + choice_map.width / 5, choice_map.y + choice_map.height*1.03))
                if self.medals[choice_map.name]["hard"]:
                    self.window.blit(self.gold_medal, (choice_map.x + choice_map.width / 3, choice_map.y + choice_map.height*1.03))
