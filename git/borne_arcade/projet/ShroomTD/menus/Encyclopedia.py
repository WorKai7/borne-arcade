...
class Encyclopedia:
    def __init__(self, screen_size, sfx_state, music_state, music) -> None:
        ...
        self.encyclopedia_text = pygame.font.SysFont('arialrounded', self.screen_size[1]//10).render('Encyclopedie', True, (0, 0, 0))
    ...
    def draw_encyclopedia(self, window) -> None:
        ...
        while running:
            ...
            window.blit(self.encyclopedia_text, (self.screen_size[0]/3.1, self.screen_size[1]/10))
            ...
            if event.type == pygame.MOUSEBUTTONDOWN:
                ...
                if self.current_window == "home":
                    ...
                    if self.enemies_button.rect.collidepoint(pygame.mouse.get_pos()):
                        play_sound("click.ogg", 1, self.sfx_state)
                        fondu_fermer(window, self.screen_size, 0.5, 50)
                        enemies_menu = Enemies_menu(self.screen_size, self.sfx_state, self.music_state, self.music)
                    ...
                    if self.physical_towers_button.rect.collidepoint(pygame.mouse.get_pos()):
                        play_sound("click.ogg", 1, self.sfx_state)
                        fondu_fermer(window, self.screen_size, 0.5, 50)
                        physical_towers_menu = Physical_towers_menu(self.screen_size, self.sfx_state, self.music_state, self.music)
                        return physical_towers_menu.run_menu(window)
                    
                    if self.magical_towers_button.rect.collidepoint(pygame.mouse.get_pos()):
                        play_sound("click.ogg", 1, self.sfx_state)
                        fondu_fermer(window, self.screen_size, 0.5, 50)
                        magical_towers_menu = Magical_towers_menu(self.screen_size, self.sfx_state, self.music_state, self.music)
