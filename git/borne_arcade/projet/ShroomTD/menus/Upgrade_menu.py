...
class Upgrade_menu:
    def __init__(self, selected_tower, screen_size:tuple) -> None:
        ...
        self.rect = pygame.transform.scale(pygame.image.load(os.path.join("assets", "other", "upgrade_rect.png")).convert_alpha(), (self.screen_size[0]/1.3, self.screen_size[1]/10))
        ...
        self.little_money_symbol = pygame.transform.scale(pygame.image.load(os.path.join("assets", "other", "money.png")).convert_alpha(), (self.rect.get_height()/3.3, self.rect.get_height()/3.3))
    ...
    def draw(self, window, current_money) -> None:
        ...
        window.blit(self.level_text, (self.selected_tower.width*1.1, self.name_text.get_height()*1.1))
        ...
        if not self.selected_tower.locked_path(0) and self.selected_tower.level[0] != "max":
            if self.first_upgrade_available and current_money >= self.selected_tower.upgrade_cost[0]:
                window.blit(self.available_rect, (self.first_upgrade_button.x, self.first_upgrade_button.y))
            else:
                window.blit(self.unavailable_rect, (self.first_upgrade_button.x, self.first_upgrade_button.y))
        ...
        if not self.selected_tower.locked_path(1) and self.selected_tower.level[1] != "max":
            if self.second_upgrade_available and current_money >= self.selected_tower.upgrade_cost[1]:
                window.blit(self.available_rect, (self.second_upgrade_button.x, self.second_upgrade_button.y))
            else:
                window.blit(self.unavailable_rect, (self.second_upgrade_button.x, self.second_upgrade_button.y))
    ...
    def update(self, selected_tower) -> None:
        ...
        self.selected_tower.range_circle = pygame.Surface((self.selected_tower.range*2, self.selected_tower.range*2), pygame.SRCALPHA).convert_alpha()
        pygame.draw.circle(self.selected_tower.range_circle, (50,50,50,128), (self.selected_tower.range,self.selected_tower.range),self.selected_tower.range)
