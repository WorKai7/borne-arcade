...
class WSshroom(Tower):
    def __init__(self, x:float, y:float, screen_size:tuple, difficulty:str) -> None:
        ...
        self.images = [pygame.transform.scale(pygame.image.load(os.path.join("assets", "towers", "water_strider_shroom", "water_strider_shroom.png")).convert_alpha(), (self.width, self.height)),
                      pygame.transform.scale(pygame.image.load(os.path.join("assets", "towers", "water_strider_shroom", "water_strider_shroom2.png")).convert_alpha(), (self.width, self.height)),
                      pygame.transform.scale(pygame.image.load(os.path.join("assets", "towers", "water_strider_shroom", "water_strider_shroom3.png")).convert_alpha(), (self.width, self.height))]
    ...
    def main_attack(self, target, enemies:list=None, current_tick:int=0, projectiles:list=None) -> list:
        ...
        if current_tick - self.last_shot_time > self.shoot_interval:
            self.animating = True
            projectiles.append(WSshroom_projectile(self.x + self.width/2, self.y + self.height/2, target, self.screen_size, self.attack_value, self.attack_type, self.speed_modifier))
