...
class Ushroom(Tower):
    def __init__(self, x:float, y:float, screen_size:tuple, difficulty:str) -> None:
        ...
        self.image = pygame.transform.scale(pygame.image.load(os.path.join("assets", "towers", "urchin_shroom","urchin_shroom.png")).convert_alpha(), (self.width, self.height))
        #self.images = [pygame.transform.scale(pygame.image.load(os.path.join("assets", "towers", "urchin_shroom", "urchin_shroom1.png")).convert_alpha(), (self.width, self.height)),
        #              pygame.transform.scale(pygame.image.load(os.path.join("assets", "towers", "urchin_shroom", "urchin_shroom2.png")).convert_alpha(), (self.width, self.height)),
        #              pygame.transform.scale(pygame.image.load(os.path.join("assets", "towers", "urchin_shroom", "urchin_shroom3.png")).convert_alpha(), (self.width, self.height)),
        #              pygame.transform.scale(pygame.image.load(os.path.join("assets", "towers", "urchin_shroom", "urchin_shroom4.png")).convert_alpha(), (self.width, self.height))]
        #self.image_index = 0
    ...
    def main_attack(self, target=None, enemies=list, current_tick=0, projectiles=None) -> list:
        ...
        if current_tick - self.last_shot_time > self.shoot_interval:
            for enemy in enemies:
                #self.animating = True
                projectiles.append(Ushroom_projectile(self.x + self.width*0.20, self.y + self.height*0.4, enemy, self.screen_size, self.attack_value, self.attack_type))
