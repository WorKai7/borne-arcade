...
class Sshroom(Tower):
    def __init__(self, x:float, y:float, screen_size:tuple, difficulty:str) -> None:
        ...
        self.image = pygame.transform.scale(pygame.image.load(os.path.join("assets", "towers", "stun_shroom", "stun_shroom.png")), (self.width, self.height))
        ...
        if current_tick - self.last_shot_time > self.shoot_interval:
            for enemy_to_stun in enemies:
                if not enemy_to_stun.Bresistance():
                    ...
                    if not self.shot:
                        enemy_to_stun.health -= int(round(self.attack_value / enemy_to_stun.resistance[self.attack_type]))
            self.shot = True
