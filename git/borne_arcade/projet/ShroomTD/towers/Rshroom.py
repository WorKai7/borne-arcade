...
class Rshroom(Tower):
    def __init__(self, x:float, y:float, screen_size:tuple, difficulty:str) -> None:
        ...
        self.images = [pygame.transform.scale(pygame.image.load(os.path.join("assets", "towers", "random_shroom", "random_shroom.png")).convert_alpha(), (self.width, self.height)),
                      pygame.transform.scale(pygame.image.load(os.path.join("assets", "towers", "random_shroom", "random_shroom2.png")).convert_alpha(), (self.width, self.height)),
                      pygame.transform.scale(pygame.image.load(os.path.join("assets", "towers", "random_shroom", "random_shroom3.png")).convert_alpha(), (self.width, self.height)),
                      pygame.transform.scale(pygame.image.load(os.path.join("assets", "towers", "random_shroom", "random_shroom2.png")).convert_alpha(), (self.width, self.height))]
        self.image_index = 0
        ...
        self.shoot_reload_time = randint(self.shoot_interval-10,self.shoot_interval+10)
    ...
    def main_attack(self, target, enemies=None, current_tick=0, projectiles=None) -> list:
        ...
        if enemies != []:
            if current_tick - self.last_shot_time > self.shoot_interval:
                ...
                self.shoot_reload_time = randint(self.shoot_interval-5,self.shoot_interval+5)
                ...
                if self.level[1] == "max":
                    ...
                    level2 = self.level[1]
                attack = int(round(4*randint(self.attack_value-(3+level1+level2),self.attack_value+(3+level1+level2))))
                projectiles.append(Rshroom_projectile(self.x + self.width//2, self.y + self.height//2, target, self.screen_size, attack, choice(self.attack_type_value)))
