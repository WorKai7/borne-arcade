...
class SPshroom(Tower):
    def __init__(self, x:float, y:float, screen_size:tuple, difficulty:str) -> None:
        ...
        self.images = [pygame.transform.scale(pygame.image.load(os.path.join("assets", "towers", "spirit_shroom", "spirit_shroom.png")).convert_alpha(), (self.width, self.height)),
                       pygame.transform.scale(pygame.image.load(os.path.join("assets", "towers", "spirit_shroom", "spirit_shroom2.png")).convert_alpha(), (self.width, self.height))]
    ...
    def main_attack(self, target, enemies=None, current_tick=0, projectiles=None) -> list:
        ...
        for enemy in enemies:
            if self.shooted.enemy_check(enemy) and (enemy not in self.enemy_damaged) and (enemy != self.current_target):
                ...
                enemy.health -= int(round(self.attack_value / enemy.resistance[self.attack_type]))
