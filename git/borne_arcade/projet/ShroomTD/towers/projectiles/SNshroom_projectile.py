...
class SNshroom_projectile(Projectile):
    def __init__(self, x:float, y:float, target, screen_size:tuple, attack_value:int, attack_type:str, resistance_modifier:float) -> None:
        ...
        self.image = pygame.transform.scale(pygame.image.load(os.path.join("assets", "towers", "projectiles", "snshroom_projectile.png")).convert_alpha(), (screen_size[0]/120, screen_size[0]/120))
        ...
        for status in self.target.status:
            if status[0] == "M.res debuff" and status[1] <= 120:
                ...
                if status[2] > self.resistance_modifier:
                    self.target.status[self.target.status.index(status)][2] = self.resistance_modifier
                return
        self.target.status.append(["M.res debuff", 120, self.resistance_modifier])