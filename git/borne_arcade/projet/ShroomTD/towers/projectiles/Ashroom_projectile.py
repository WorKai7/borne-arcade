...
class Ashroom_projectile(Projectile):
    def __init__(self, x:float, y:float, target, screen_size:tuple, attack_value:int, attack_type:str, resistance_modifier:float) -> None:
        ...
        self.image = pygame.transform.scale(pygame.image.load(os.path.join("assets", "towers", "projectiles", "ashroom_projectile.png")).convert_alpha(), (screen_size[0]/120, screen_size[0]/120))
    ...
    def attack(self) -> None:
        ...
        for status in self.target.status:
            if status[0] == "P.res debuff" and status[1] <= 120:
                ...
                return
        self.target.status.append(["P.res debuff", 120, self.resistance_modifier])