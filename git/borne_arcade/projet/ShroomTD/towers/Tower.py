...
class Tower:
        ...
        if self.level[path] != "max" and not self.locked_path(path):
            if current_money >= self.upgrade_cost[path]:
                ...
                if self.level[path] == 5:
                    ...
                    self.upgrade_cost[path] = self.upgrades[self.name][str(path)][str(self.level[path]+1)]["upgrade_cost"]
    ...
    def main_attack(self):
        ...
        raise NotImplementedError("La méthode n'est pas défini pour cette classe")
