...
class Wshroom(Tower):
    def __init__(self, x:float, y:float, screen_size:tuple, difficulty:str) -> None:
        ...
        self.images = [pygame.transform.scale(pygame.image.load(os.path.join("assets", "towers", "wizard_shroom", "wizard_shroom.png")).convert_alpha(), (self.width, self.height)),
                      pygame.transform.scale(pygame.image.load(os.path.join("assets", "towers", "wizard_shroom", "wizard_shroom2.png")).convert_alpha(), (self.width, self.height)),
                      pygame.transform.scale(pygame.image.load(os.path.join("assets", "towers", "wizard_shroom", "wizard_shroom3.png")).convert_alpha(), (self.width, self.height)),
                      pygame.transform.scale(pygame.image.load(os.path.join("assets", "towers", "wizard_shroom", "wizard_shroom4.png")).convert_alpha(), (self.width, self.height))]
        self.image_index = 0
    ...
    def main_attack(self, target, enemies=None, current_tick=0, projectiles=None) -> list:
        ...
        if current_tick - self.last_shot_time > self.shoot_interval:
            ...
            while hit < self.max_hit and target != None:
                ...
                enemy_number = 0
                self.animating = True
                projectiles.append(Wshroom_projectile(self.x + self.width*0.8, self.y + self.height*0.5, target, self.screen_size, self.attack_value, self.attack_type))
                hit += 1
