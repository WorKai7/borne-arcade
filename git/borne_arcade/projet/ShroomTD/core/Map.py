...
class Map:
    def __init__(self, nom_map:str, screen_size:tuple) -> None:
        ...
        self.bg = pygame.transform.scale(pygame.image.load(os.path.join("assets", "maps", self.maps[nom_map]["asset"])).convert(), (self.width, self.height*0.9))
        ...
        try:
            self.music = pygame.mixer.Sound(os.path.join("assets", "sounds", "musics", self.maps[nom_map]["music"]))
        except pygame.error:
            self.music = None
    ...
    def convert_json_maps(self) -> None:
        ...
        for map_name, mapdata in self.maps.items():
            for data_name, data_value in mapdata.items():
                if data_name == "start":
                    self.maps[map_name]["start"] = (data_value[0]*self.unitL, data_value[1]*self.unit)
                if data_name == "path":
                    for coord in data_value:
                        if self.nom_map == "meadow":
                            self.maps[map_name]["path"][self.maps[map_name]["path"].index(coord)] = (coord[0]*self.unitL, coord[1]*self.unit)
                        else:
                            self.maps[map_name]["path"][self.maps[map_name]["path"].index(coord)] = (coord[0]*self.unitL + (self.unitL*0.15), coord[1]*self.unit)
                if data_name == "tower_placement":
                    if data_value != [[]]:
                        for coord in data_value:
                            if self.nom_map == "meadow":
                                self.maps[map_name]["tower_placement"][self.maps[map_name]["tower_placement"].index(coord)] = (coord[0]*self.unitL, coord[1]*self.unit)
                            else:
                                self.maps[map_name]["tower_placement"][self.maps[map_name]["tower_placement"].index(coord)] = (coord[0]*self.unitL + (self.unitL*0.15), coord[1]*self.unit)
                if data_name == "water_placement":
                    if data_value != [[]]:
                        for coord in data_value:
                            if self.nom_map == "meadow":
                                self.maps[map_name]["water_placement"][self.maps[map_name]["water_placement"].index(coord)] = (coord[0]*self.unitL, coord[1]*self.unit)
                            else:
                                self.maps[map_name]["water_placement"][self.maps[map_name]["water_placement"].index(coord)] = (coord[0]*self.unitL + (self.unitL*0.15), coord[1]*self.unit)
